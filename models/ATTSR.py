...
class PatchMerging3x3(nn.Module):
    r""" 3x3 Patch Merging Layer.
    ...
    def __init__(self, input_resolution, dim, norm_layer=nn.LayerNorm):
        ...
        self.dim = dim
        # 将9 * dim压缩到3 * dim
        self.reduction = nn.Linear(9 * dim, 3 * dim, bias=False)
        self.norm = norm_layer(9 * dim)
    ...
    def forward(self, x, x_size):
        ...
        B, L, C = x.shape
        assert L == H * W, "input feature has wrong size"
        assert H % 3 == 0 and W % 3 == 0, f"x size ({H}*{W}) are not divisible by 3."

        # 将输入特征图重构成 B, H, W, C
        x = x.view(B, H, W, C)

        # 切分成3x3的小块
        x0 = x[:, 0::3, 0::3, :]  # B, H/3, W/3, C
        x1 = x[:, 1::3, 0::3, :]  # B, H/3, W/3, C
        x2 = x[:, 2::3, 0::3, :]  # B, H/3, W/3, C
        x3 = x[:, 0::3, 1::3, :]  # B, H/3, W/3, C
        x4 = x[:, 1::3, 1::3, :]  # B, H/3, W/3, C
        x5 = x[:, 2::3, 1::3, :]  # B, H/3, W/3, C
        x6 = x[:, 0::3, 2::3, :]  # B, H/3, W/3, C
        x7 = x[:, 1::3, 2::3, :]  # B, H/3, W/3, C
        x8 = x[:, 2::3, 2::3, :]  # B, H/3, W/3, C

        # 将这9个子区域合并
        x = torch.cat([x0, x1, x2, x3, x4, x5, x6, x7, x8], dim=-1)  # B, H/3, W/3, 9*C
        x = x.view(B, -1, 9 * C)  # B, H/3*W/3, 9*C

        # 归一化和线性映射
        x = self.norm(x)
        x = self.reduction(x)  # 压缩到 3 * dim

        return x, (x_size[0] // 3, x_size[1] // 3)
    ...
    def flops(self):
        H, W = self.input_resolution
        flops = H * W * self.dim  # 归一化操作
        flops += (H // 3) * (W // 3) * 9 * self.dim * 3 * self.dim  # 线性层操作
        return flops
...
class CrossAttention(nn.Module):
    def __init__(self, dim, y_dim, num_heads, bias):
        ...
        self.y_qkv_dwconv = nn.Conv2d(y_dim * 2, y_dim * 2, kernel_size=3, stride=1, padding=1, groups=y_dim * 2,
                                      bias=bias)
...
class ATTSR(nn.Module):
    def __init__(self, args, img_size=32, img_size_y=128, scale_factor=3, patch_size=1, in_chans=1, out_chans=1,
                 in_chans_y=3,
                 embed_dim=64, de_features=64, embed_dim_y=64, coex_features=64, up_heads=[1, 2], num_heads=[4, 2],
                 depths=[6, 6, 6, 6], guid_depths=[6, 6, 6, 6],
        ...
        self.Fconv = nn.Sequential(nn.Conv2d(3, self.de_features, kernel_size=5, stride=1, padding=2),
        ...
        dpr_d = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))]
        ...
        dpr = [x.item() for x in
               torch.linspace(0, drop_path_rate, sum(self.down_depths))]
        ...
        self.guidedlayer0 = BasicLayer(
            ...
            norm_layer=norm_layer,
            use_checkpoint=use_checkpoint)
        self.guidedlayer1 = BasicLayer(dim=int(embed_dim_y * scale_factor),
                                       input_resolution=(patches_resolution[0],
                                                         patches_resolution[1]),
                                       depth=guid_depths[1],
                                       num_heads=num_heads[0],
                                       ...
                                       norm_layer=norm_layer,
                                       use_checkpoint=use_checkpoint)
        self.guidedlayer2 = BasicLayer(dim=int(embed_dim_y * scale_factor),
                                       input_resolution=(patches_resolution[0],
                                                         patches_resolution[1]),
                                       depth=guid_depths[2],
                                       num_heads=num_heads[0],
        ...
        self.guidedlayer3 = BasicLayer(dim=int(embed_dim_y * scale_factor),
                                       input_resolution=(patches_resolution[0],
                                                         patches_resolution[1]),
                                       depth=guid_depths[3],
        ...
        self.crosslayer_d0 = CrossTransformerblock(dim=int(embed_dim),
                                                   y_dim=int(embed_dim_y * scale_factor),
                                                   input_resolution=(patches_resolution[0],
                                                                     patches_resolution[1]),
        ...
        self.crosslayer_c0 = CrossTransformerblock(dim=int(embed_dim_y * scale_factor),
                                                   y_dim=int(embed_dim),
                                                   input_resolution=(patches_resolution[0],
                                                                     patches_resolution[1]),
                                                   num_heads=num_heads[1],
        ...
        self.before_upconv = nn.Sequential(
            nn.Conv2d(in_channels=4 * embed_dim, out_channels=64, kernel_size=1, stride=1, padding=0),
            nn.PReLU()
        )
        ...
        self.before_upconv_c = nn.Sequential(
            nn.Conv2d(in_channels=int(embed_dim_y * scale_factor), out_channels=64, kernel_size=1, stride=1,
                      padding=0),
            nn.PReLU()
        )
    ...
    def forward(self, x, y):
        ...
        x3 = self.crosslayer_d0(x2, y2, x_size)
        x3, x_size = self.depthlayer2(x3, x_size)

        y3 = self.crosslayer_c0(y2, x3, x_size)
        y3, y_size = self.guidedlayer2(y3, y_size)

        x4 = self.crosslayer_d0(x3, y3, x_size)
        x4, x_size = self.depthlayer3(x4, x_size)

        y4 = self.crosslayer_c0(y3, x4, x_size)
...
if __name__ == '__main__':
    ...
    hr_height, hr_width = 96, 96

    input = torch.rand(batch_size, channels, lr_height, lr_width)
    color = torch.rand(batch_size, color_channels, hr_height, hr_width)

    dem_sr, cdepth_sr, fudepth_sr = model(input, color)

    print(dem_sr.shape, fudepth_sr.shape, cdepth_sr.shape)
